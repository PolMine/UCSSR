...
- Die CWB beinhaltet als Bündel von Tools mit dem *Corpus Query Processor* (CQP) ein mächtiges Instrument, für große Korpora komplexe sprachlich Suchanfragen zur formulieren. Queries können reguläre Ausdrücke beinhalten und linguistische Annotationen abfragen.
...
- Das Datenformat ist grundsätzlich tabellarisch. Wnn neben der Ursprünglichen Wortform (p-Attribut "word") auch ein Wortarterkennung (sog. "Part-of-Speech"-Annotation, p-Attribut "pos") und eine Lemmatisierung (p-Attribute "lemma") durchgeführt wurde, so ist ein dreispaltiger *tokenstream* verfügbar.

- Die Tabelle auf der folgenden Seite gibt als Beispiel den Anfang einer Plenardebatte wider. In der erseten ist die *corpus position* ("cpos") angeführt.
