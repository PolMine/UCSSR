...
* Please note: The functionalities explained here were tested for the current polmineR version `r as.package_version("0.8.6")`. Install the correct version of the package accordingly.
...
kwic(unga_2005, 
     query = "immigration",
     s_attributes = "state_organization", 
     verbose = FALSE)
```
...
kwic(unga_2005, 
     query = "immigration", 
     s_attributes = c("state_organization", "date"), 
     verbose = FALSE)
```
...
K <- kwic(unga_2005, query = "immigration", s_attribute = "state_organization")
read(K, i = 1)
```
...
f <- fulltext::as.fulltexttable(P, headline = "State or Organization: International Organization for Migration", display = "block", interjections = FALSE)
