...
* Die im PolMine-Projekt aufbereiteten Korpora werden aus Ausgangsformaten (pdf, plain text, html) in standardisierte XML-Formate übersetzt. Die Standardisierung entspricht Vorgaben der [Text Encoding Initiative (TEI)](http://www.tei-c.org/). 
...
Die `corpus()`-Methode (ohne Argumente) gibt eine Liste der Korpora an, auf die in Analysen zugegriffen werden kann. Die Tabelle gibt in der zweiten Spalte die Korpusgröße an. Der Spalte "template" gibt Auskunft, ob Regeln zur Formatierung bei Volltextanzeigen verfügbar sind. Indem wir corpus() aufrufen, können wir prüfen, ob das GERMAPARL-Korpus verfügbar ist. Das GERMAPARL-Korpus wird erst nach dem Herunterladen und der Installation mit dieser Methode angezeigt. Die Korpora "REUTERS" und "GERMAPARLMINI" sind hingegen nach der Installation von polmineR verfügbar.
...
REUTERS und GERMAPARLMINI sind Korpora, die als Beispieldaten im polmineR-Paket enthalten sind. Das GERMAPARL-Korpus ist nach dem Laden des polmineR-Pakets ebenfalls verfügbar, womit die folgenden Analysen durchgeführt werden können.
...
`r fontawesome::fa("lightbulb")`  Grundsätzlich ist diese Datenstruktur vergleichbar mit jener, die Sie vielleicht auch vom tidytext-package kennen.
...
Oben wurde schon erwähnt, dass Sie mit der `size()`-Methode die Größe eines Korpus abfragen können.
