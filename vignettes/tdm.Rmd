...
```{r, message = FALSE}
for (pkg in c("magrittr", "slam", "tm", "quanteda")){
...
- Die `as.DocumentTermMatrix()`-Methode kann auch auf `partition`-Objekte angewendet werden.

```{r, eval = FALSE}
bt16 <- partition("GERMAPARL", lp = 16, interjection = FALSE)
dtm <- polmineR::as.DocumentTermMatrix(bt16, p_attribute = "word", s_attribute = "speaker")
```
...
dtm <- t(polmineR::as.sparseMatrix(pb, col = "count"))
```
...
```{r, echo = TRUE}
textplot_scale1d(wfm_1, doclabels = pg_dfm2@Dimnames$docs)
```
...
Wird hier primär der Gegensatz Regierung zu Opposition skaliert? Eine thematisch engere Auswahl von Debatten und eine zielgerichtetere Reduktion der Matrix kann hier interessantere Ergebnisse bringen. 
