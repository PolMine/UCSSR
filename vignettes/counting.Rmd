...
- Es ist möglich, die `partition()`-Methode und die `count()`-Methode in einer "Pipe" zu verbinden (Diese wird durch das magrittr Paket bereitgestellt, das mit polmineR installiert wird). Dies bedeutet, Funktionen oder Methoden mit dem Pipe-Operator ("%>%") zu verketten, wobei dann der Rückgabewert einer Methode zum (ersten) Argument der darauffolgenden Methode werden kann.
