...
The polmineR package is consistently improved. The most recent development version is available on [GitHub](https://github.com/PolMine/polmineR), a plattform to administrate and share open-source software. The development version fixes known bugs and contains new functionilites as well as improved documentation. The installation is performed using `install_github()` from the `devtools` package.
